...
func (registry *Registry) DownloadBlob(repository string, digest digest.Digest) (io.ReadCloser, error) {
	...
	registry.Logger.Infof(context.TODO(), "registry.blob.download url=%s repository=%s digest=%s", url, repository, digest)
...
func (registry *Registry) UploadBlob(repository string, digest digest.Digest, content io.Reader) error {
	...
	registry.Logger.Infof(context.TODO(), "registry.blob.upload url=%s repository=%s digest=%s", uploadURL, repository, digest)
...
func (registry *Registry) HasBlob(repository string, digest digest.Digest) (bool, error) {
	...
	registry.Logger.Infof(context.TODO(), "registry.blob.check url=%s repository=%s digest=%s", checkURL, repository, digest)
...
func (registry *Registry) initiateUpload(repository string) (*url.URL, error) {
	...
	registry.Logger.Infof(context.TODO(), "registry.blob.initiate-upload url=%s repository=%s", initiateURL, repository)
